...
class StockReportGV(ScraperComponent):
    def __init__(self, name: str):
        ScraperComponent.__init__(self, name)

    def scrap(self, wd: WebDriver, ref: DataRef) -> Set[Data]:
        data = common.init_data('Growth & Value', self.name, wd, ref)
        tables = wd.find_elements_by_css_selector("stockreport-ratios.valueComp table")
        for table in tables:
